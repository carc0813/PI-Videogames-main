...
router.get("/", async (req, res) => {
  try {
    const getVidegamesApi = async () => {
      ...
      //console.log(apiUrl)
      try { (promise1 = axios.get( `https://api.rawg.io/api/games?key=${API_KEY}&page=1&page_size=50` )),
        ...
        .then( (values) => {apiResult = values[0].data.results
              ...
              return {
                ...
                genres:videos.genres && videos.genres.map((p) => p.name).filter((p) => p != null).join(", "),
                origin: 'API'
              };
            });
            return apiResult;
